...
### Silence stops a recording that nobody stopped
A recording nobody is speaking into used to run until somebody noticed — one session here went from 19:00 to 05:48 and wrote 8.6 GB. When neither the microphone nor the Mac's own audio has carried speech for a while, a small window asks whether to carry on. **Keep recording** restarts the clock, so the same wait asks again rather than never asking twice; with no answer the recording stops by itself, is transcribed exactly as a manual stop would be, and the transcript's last line says it stopped automatically and after how long. Both intervals are in Settings → Meetings → Silence (five minutes and two, by default). If speech resumes while the question is up, the question goes away and the recording continues. ([SilenceWatch.swift](Sources/Griasa/SilenceWatch.swift), [SilenceLevel.swift](Sources/Griasa/SilenceLevel.swift))

### One hub window instead of popup sprawl
Everything lives in a single floating **hub window with tabs**: action results, the live Recording tab, the Custom-reminder date picker, the "who was on the call?" question, Ask Project, and History. A busy moment — recording a call while setting a reminder off a preset result — is one window with three tabs, not three windows fighting for focus. Closing a tab resolves its flow properly (a dismissed reminder deletes its orphan clip; closing the participants tab counts as "Skip" so the transcription pipeline never stalls). The **pin** beside the tabs keeps the window open when you switch apps — a panel hides itself on deactivation by default, which suits a popup and not an hour spent reading history. Only Settings (the standard macOS settings scene) and system dialogs (file save/open, the region-drag capture overlay) stay outside. ([HubWindow.swift](Sources/Griasa/HubWindow.swift))
